...
def create_terraform_import(resource_type):
    if input(f"\nWould you like to create the imported {resource_type} into Terraform? (y/n) ").lower() == "y":
        file_path = f'okta_{resource_type}.json'
        try:
            with open(file_path, 'r') as file:
                data = json.load(file)
            create_terraform_import_script(data, resource_type)
    ...
    with open(output_file, 'w') as file:
        ...
        for item in data:
            if resource_type == 'groups':
                ...
                id = item['id']
                resource_address = f"okta_group.{name}"
            elif resource_type == 'users':
                name = f"{item['profile']['firstName']} {item['profile']['lastName']}"
                id = item['id']
